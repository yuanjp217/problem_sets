...
for (var in 1:n) {
  #print(var)
  if (var == 1) 
    {first <- t.test((rnorm(30)))
    pvals_for <- first$p.value}
  else
    {output <- t.test((rnorm(30)))
    pvals_for <- as.data.frame(append(list,output$p.value))}
}

sig_for <- pvals_for %>%
  filter(pvals_for < 0.05)
...
```{r}
n = 10000
pvals <- as.data.frame(replicate(n,t.test(rnorm(30))$p.value))
sig <- pvals %>% 
  filter(pvals < 0.05)
sig
count(sig)/n
