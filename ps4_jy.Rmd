...
for (var in 1:n) {
  #print(var)
    ...
    #pvals_for <- as.data.frame(append(pvals_for,output$p.value))
  }
...
> About 5% of the samples show up as significant results. 
...
```{r}
double.sample <- function (tests) {
  
#testOutputs <- (replicate(tests,t.test(rnorm(30))))

test <- t.test(rnorm(30)) 
pval <- test$p.val
print(pval)
